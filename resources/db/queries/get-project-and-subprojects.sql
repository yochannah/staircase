...
WITH all_projects(
    ...
    created_at, last_accessed, last_modified, parent_id) AS
    SELECT id, title, description,
        created_at, last_accessed, last_modified, parent_id
    FROM projects
    WHERE parent_id IS NULL -- must be top-level.
      AND id = :project -- primary key.
      AND owner = :owner -- For access control.
  UNION ALL
    SELECT sp.id, sp.title, sp.description,
        sp.created_at, sp.last_accessed, sp.last_modified, sp.parent_id
